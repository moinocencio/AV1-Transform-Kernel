...
When considering the transformation stage, a widely used approach is the use of \emph{integer transforms}. However, these Doing so maintains the features of floating point transforms, but reduces the used operations to arithmetic additions and multiplications (or shifts).

From the previously presented transforms, there have been several methods of developing integer counterparts. In \emph{libaom}, this was achieved with scaling and approximating the basis images to a certain scale, depending of the number of used bits. However, other fast implementations have been developed, most of them being based in either \emph{Fast Fourier Transform} algorithms or in the \emph{Walsh-Hadamard Transform} \cite{wolterParallelArchitecturesDiscrete1992,yonghongzengIntegerDCTsFast2001}.

Most of the faster implementations tend to present higher distortion rates, due to the implemented approximations. This aspect \textcolor{red}{can be observed in figure \ref{fig:lena}.} However, since most of the distortion comes from the \emph{Quantization} stage, these small errors tend to be ignored.

\todo[inline,color=red!40]{Reconstruction of image with regular vs integer DCT and respective error images}

\begin{figure}[h]
    \centering
    \missingfigure{Lena with IntDCT and DCT reconstruction}
    \caption{Effect of integer DCT}
    \label{fig:lena}
\end{figure}
