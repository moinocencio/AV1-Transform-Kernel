...
\begin{table}[!htpb]
    ...
    \begin{tabular}{cLL} \toprule
        ...
        1 &         \multicolumn{1}{m{0.35\textwidth}}{Repeat $g(n)$ every L points, giving origin to $\widetilde{g}_L(n)$} &         \multicolumn{1}{m{0.35\textwidth}}{Concatenate $g(n)$ with a flipped version of itself, creating a $2L$ sequence, $g_{2L}(n)$, and repeat it, giving origin to $\widetilde{g}_{2L}(n)$}\\ \hline
...
Most of the intermediary coefficients inside each stage are calculated with the function \verb|half_btf|, which performs the operation represented in Equation \ref{eq:half_btf}. This function takes the two previously calculated coefficients, two values from the previously introduced approximations arrays, as well as the number of bits used to represent these, and maps the result from the multiplications and sum of the first inputs to the desired number of bits.
