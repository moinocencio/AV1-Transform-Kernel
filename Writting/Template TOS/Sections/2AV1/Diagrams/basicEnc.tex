...
\coordinate (aboveec) at (EC |- control);
    ...
    \draw [aref] (inter.south) -- ++(0,-3.2mm) |- ($(inter.south east)+(22mm,-3.2mm)$) coordinate (y) |- (y |- control.south);
    \draw [aref] (sel.east) -- ++(4mm,0) coordinate (y) |- (y |- control.south);

%% Legend
\node [right=1.5cm of rq1, font={\tiny}] (lc) {Control\\Signal};
    \draw [aref] ($(lc.west)+(-6mm,0)$) -- +(5mm,0);
\node [above=7mm of lc, font={\tiny}] (lb) {Feedback\\Loop};
    \draw [fback] ($(lc.west)+(-6mm,7mm)$) -- +(5mm,0);
\node [above=7mm of lb, font={\tiny}] (lf) {Forward\\Path};
    \draw [fforw] ($(lc.west)+(-6mm,14mm)$) -- +(5mm,0); 
